...
class BoxConfig:
    ...
    def save(self):
        ...
        with open(config_file_path, 'w') as csv_file:
            ...
            writer.writerow(
                ["output_data_rate", "bluetooth_scan_time", "magnetic_map_interpolation", "magnetic_map_color",
                 "server_ip", "server_port"])
            writer.writerow([self.output_data_rate, self.bluetooth_scan_time, self.magnetic_map_interpolation,
                             self.magnetic_map_color, self.server_ip, self.server_port])
...
def load_conf():
    ...
    return (
    {'odr': odr, 'cali': nume, 'inter': inter, 'colormap': colormap, 'server': server, 'port': port, 'tempo': tempo})

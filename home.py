...
tempo = 1		#secondi di ricerca ble
...
def config():
    ...
    if request.method == 'POST':
        datare = request.json
        jc = configf.save_conf(datare['odr'],datare['cali'],datare['inter'],datare['colormap'],datare['ble'],datare['server'],datare['port'])
        odr = jc['odr']
        nume = jc['cali']
        inter = jc['inter']
        ...
        print("tempo_ble = ", tempo)
        print("server_sincro = ", server)
        print("porta_sincro = ", port_server)
        print("nume_cal = ", nume)
        print("inter = ", inter)
        print("colormap = ", colormap)
        if odr == 625:
...
def sm():
    ...
    if k == 'b':
        return render_template('/sm.html', x="{:.3f}".format(x), y="{:.3f}".format(y), z="{:.3f}".format(z), c="{:.3f}".format(c))
    else:
        return jsonify({"x":"{:.3f}".format(x), "y":"{:.3f}".format(y), "z":"{:.3f}".format(z), "c":"{:.3f}".format(c)})
...
def nuova_calibrazione():
    ...
    calibration_thread = threading.Thread(target=nuova_calibrazione_thread_func, args=(total_intensity, sample_frequency, ))
...
def meas():
    prnt()
    bluetooth_scan_entries = ble.get_available_devices(tempo)
    wifi_scan_entries = wifis.Scan()
    
    print(wifi_scan_entries)
    ...
    if(ble_s == 1):
        try:
            ...
            if len(bluetooth_macs) == 0:
                ...
                dble = ble.scan_filter(tempo, bluetooth_macs)
...
def end():
    ...
    url_img=heatmap.magnmap(data, token, inter, colormap)
    return "ok"
...
def down_pac():
    ...
    resusincro = sincro.download(server, port_server, tt, nn, dd)
    return jsonify(resusincro)

@app.route("/up")
def up():
    tt = request.args.get('t')
    resusincro = sincro.upload(server, port_server, tt)
    return jsonify(resusincro)
...
def elencodatiremoto():
    v=dati.load_remot(server, port_server)
    return (v)
...
def get_markers():
    ...
    bn = os.getcwd() + upl_dir + token + '/misu.txt'
    b = pd.read_csv(bn)
    finalresponse = [{"idp": ii,"lat": aa, "long": bb, "x": cc, "y": dd, "misu":ee} for ii, aa, bb, cc, dd, ee in zip(b['idp'], b['lat'], b['long'], b['x'], b['y'], b['misu'])]
    print(finalresponse)
    return jsonify(finalresponse)
